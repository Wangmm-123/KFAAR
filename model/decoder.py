...
class StyledGenerator(nn.Module):
    ...
    def make_style(self, z, mean_style=None, style_weight=0):
        style = self.style(z)
        if mean_style != None:
    ...
    def forward(
        ...
        for i in range(step + 1):
            size = 4 * 2 ** i
            if not noise:
                n.append(torch.zeros(batch, 1, size, size, device=input[0].device))
            else:
                n.append(torch.randn(batch, 1, size, size, device=input[0].device))
        noise = n
