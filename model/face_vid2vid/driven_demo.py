...
from model.face_vid2vid.sync_batchnorm import DataParallelWithCallback
from model.face_vid2vid.modules.generator import OcclusionAwareGenerator, OcclusionAwareSPADEGenerator
from model.face_vid2vid.modules.keypoint_detector import KPDetector, HEEstimator
from model.face_vid2vid.animate import normalize_kp
...
def make_animation(source_image, driving_video, generator, kp_detector, he_estimator, relative=True, adapt_movement_scale=True, estimate_jacobian=True, cpu=False, free_view=False, yaw=0, pitch=0, roll=0):
    with torch.no_grad():
        ...
        # source1 = source.permute(0, 3, 1, 2)
        ...
        # driving = driving_video.unsqueeze(0).unsqueeze(0)
        driving = driving_video.unsqueeze(0).unsqueeze(0).permute(0,2, 1, 3, 4)
        ...
        # return predictions
        prediction = [(pred * 255).astype(np.uint8) for pred in predictions]
        pre = torch.tensor(prediction).squeeze()
        return pre
